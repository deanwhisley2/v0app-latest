...
export class TelegramNotifier {
  ...
  private loadConfig(): void {
    ...
    if (botToken && chatId) {
      ...
      console.warn("[TelegramNotifier] TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID not set. Telegram notifications disabled.")
  ...
  async sendToChat(text: string, chatId: number | string): Promise<boolean> {
    ...
    try {
      ...
      console.log("[TelegramNotifier] ✅ Message sent to Telegram")
      return true
    } catch (error) {
  ...
  async sendCoinsReport(
    ...
    const recommended = results.filter((r) => r.recommendation === "STRONG BUY" || r.recommendation === "BUY").length
    ...
    lines.push(`🟢 Recommended: ${recommended} | 🟡 Neutral: ${neutral} | 🔴 Avoid: ${avoided}`)
    lines.push("")
    ...
    for (const r of sorted) {
      ...
      lines.push(`   Score: ${r.confidenceScore}/100 | PnL: $${r.avgPnl} | Latency: ${r.avgLatencyMs}ms`)
    }
    ...
    for (const r of topCoins) {
      lines.push(
        `🟢 *${r.symbol}*: ${r.confidenceScore}/100 confidence`
      )
      lines.push(`   ${r.winRate}% win rate | ${r.totalTrades} trades | Avg PnL: $${r.avgPnl}`)
    }
   ...
   */
  async sendAlert(title: string, message: string, severity: "info" | "warning" | "critical" = "info"): Promise<boolean> {
