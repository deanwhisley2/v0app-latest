...
const LOGIN_JOELIN_CHIPS = [
  {
    label: "First steps after login",
    prompt: "What should I do first after I sign in to the dashboard?",
  },
  {
    label: "Wallet & withdrawals",
    prompt: "Explain Nexus Main wallet rules and how withdrawals work.",
  },
  {
    label: "Human support",
    prompt: "How do I reach a human assistant or official support?",
  },
];
...
export default function LoginForm() {
  ...
  useEffect(() => {
    try {
      ...
      setPostLoginPath(
        sanitizeInternalRedirect(params.get("next")) ?? "/dashboard",
      );
    } catch {
      /* ignore */
    }
    ...
    try {
      ...
      if (!loginRes.ok) {
        ...
        if (msg.toLowerCase().includes("invalid login credentials")) {
          setError(
            "Wrong phone or password. Check caps lock or tap Forgot password.",
          );
        } else if (msg.toLowerCase().includes("no account")) {
          setError(
            "No account has this phone linked. Open a new Nexus account or sign in with your email.",
          );
        } else {
          setError(msg);
        }
  ...
  const loginFooter = (
    <div className="mt-6 space-y-2">
      ...
      >
        <p>
          Use your own verified phone number and avoid shared or public devices.
        </p>
        <p className="mt-1.5">
          Never share your password or 6-digit Nexus Security PIN with anyone.
        </p>
        <p className="mt-1.5">
          Confirm you are on{" "}
      ...
      <footer className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 pt-2 text-center text-[11px] text-muted-foreground">
        <Link
          href="/legal/terms"
          className="min-h-[44px] inline-flex items-center hover:text-foreground"
        >
          {t.footer.terms}
        </Link>
        <Link
        ...
        >
          <div className="space-y-2.5">
            ...
            <p className="text-xs leading-relaxed text-muted-foreground">
              Registered with email? Enter your email address here — then add
              your phone in Settings for faster sign-in next time.
            </p>
          </div>
          ...
          {sessionCleared ? (
            <p
              className="rounded-xl border border-sky-500/40 bg-sky-500/10 px-3 py-2.5 text-xs text-sky-100"
              role="status"
            >
          ...
          {sessionRequired ? (
            <p
              className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-3 py-2.5 text-xs text-amber-100"
              role="status"
            >
          ...
          {resetSuccess ? (
            <p
              className="rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-3 py-2.5 text-xs text-emerald-300"
              role="status"
            >
          ...
          {error ? (
            <p
              className="rounded-xl border border-destructive/50 bg-destructive/10 px-3 py-2.5 text-xs text-destructive"
              role="alert"
            >
              {error}
            </p>
